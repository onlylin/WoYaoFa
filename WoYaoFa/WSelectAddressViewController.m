//
//  WSelectAddressViewController.m
//  WoYaoFa
//
...
@interface WSelectAddressViewController (){
    NSInteger currentPage;
}

@end

@implementation WSelectAddressViewController
@synthesize addressBookType;
...
- (void)viewDidLoad {
    ...
    if (addressBookType == AddressBookTypeShipper) {
        self.navigationItem.title = @"选择发货人地址";
    }else{
        self.navigationItem.title = @"选择收货人地址";
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    WAddressBookView *addressBookView = [self.viewModels objectAtIndex:indexPath.section];
    [[NSNotificationCenter defaultCenter] postNotificationName:WNotificationSelectAddress object:addressBookView.model];
    [self.navigationController popViewControllerAnimated:YES];
}

- (void)getAddressBooks{
    RACSignal *signal = [[LinApiManager shareInstance] getAddressBooks:1 addressType:addressBookType pageIndex:currentPage pageSize:1];
